...
- Padding is POST-padding (zeros appended after the real tokens).
  This is consistent with pack_padded_sequence usage in later phases where
  we will pass sequence lengths to PyTorch's packing utilities.
...
class TicketDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Retrieve the encoded (input_ids, label) pair at position idx.
        ...
        Processing pipeline per sample:
            ...
            7. Return (LongTensor[max_len], LongTensor scalar).
